...
function TourInfo() {
  ...
  useEffect(() => {
    async function fetchInitialData() {
      try {
        const res = await fetch(buildApiUrl(API_ENDPOINTS.TOUR_INFO));
  ...
  const handleGenerateReport = async (e) => {
    ...
    try {
      const res = await fetch(
        buildApiUrlWithQuery(API_ENDPOINTS.TOUR_INFO, {
          site: filters.site || "",
          fromDate: filters.fromDate || "",
          toDate: filters.toDate || "",
          holidays: filters.holidays || "",
      ...
      console.log("Tour info response:", response);

      if (response.success) {
        setTourData(response.data.tourInfo || []);
  ...
  // Search filter
  const filteredData = tourData.filter((record) => {
  ...
  return (
    <div className="tourinfo-container">
      <h1 className="tourinfo-title">Applied Tour Leaves Information</h1>
      ...
      {tourData.length > 0 && (
        <div className="stats-section">
          <div className="stat-card">
            ...
            <span className="stat-value">{statistics.totalTours}</span>
          </div>
          <div className="stat-card approved">
            <span className="stat-label">Approved:</span>
            <span className="stat-value">{statistics.approvedTours}</span>
          </div>
          <div className="stat-card pending">
            <span className="stat-label">Pending:</span>
            <span className="stat-value">{statistics.pendingTours}</span>
          </div>
        </div>
      )}
                ...
                {currentData.length > 0 ? (
                  currentData.map((record, index) => (
                    <tr key={`${record.tour_id}-${index}`}>
                      ...
                      <td>{record.begda}</td>
                      <td>{record.endda}</td>
                      <td>{record.total_tour_days}</td>
                      <td>{record.adjusted_total_tour_days}</td>
                      <td>
                        <span
      ...
      {!loading && tourData.length === 0 && !error && (
        <div style={{ textAlign: "center", padding: "2rem", color: "#666" }}>
          <p>Select filters and click "Generate Report" to view tour data</p>
        </div>
      )}
    </div>
  );
